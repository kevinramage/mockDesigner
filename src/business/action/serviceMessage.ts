...
export class ServiceMessage implements IServiceAction {
    ...
    public generate(tab: string) {
        ...
        if ( this._bodyFileName ) {
            code += tab + util.format("ResponseHandler.sendContentFromFile(context, res, %s, \"%s\", headers);\n", this.status, this.bodyFileName);
        }

        // Send a body
        else {
            code += tab + util.format("ResponseHandler.sendContent(context, res, %s, \"%s\", headers);\n", this.status, this.body);
        }
